...
class EasyRiderStage5(StageTest):
    def generate(self) -> List[TestCase]:
        return [
            TestCase(
                stdin='[{"bus_id" : 512, "stop_id" : 4, "stop_name" : "Bourbon Street", "next_stop" : 6, "stop_type" : "S", "a_time" : "08:13"}, '
        ...
        if isinstance(result, str):
            ...
            for x in range(len(result[0])):
                ...
                if not response or sum(
                        response.group(0).count(txt) for txt in ("Avenue", "Street", "Boulevard", "Road")) != 1:
                    return CheckResult.wrong("Wrong number of bugs detected or the output format is wrong in bus line "
                                             f"{result[0][x][0]}. Expected output:\n\n" 
                                             f"bus_id line {result[0][x][0]}: wrong time on station {result[0][x][1]}")
